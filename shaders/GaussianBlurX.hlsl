...
float4 main(float2 texCoord: TEXCOORD0) : COLOR0
{
 ...
 //   return vAccum;
    ...
    return BlurF(texCoord, float2(1, 0), BloomBlurSigma);
  ...
  //  {
  //      Value += tex2D(Sampler, float2(texCoord.x + (Index - 10) * TextelScale, texCoord.y)) * Coefficients[Index];
  //  }
  ...
  //  return Value;
    ...
    //for (int i = 0; i < 13; ++i)
    //{
    //    coord.y = texCoord.y + (sBlur * Kernel[i]) / 1920.0f;
