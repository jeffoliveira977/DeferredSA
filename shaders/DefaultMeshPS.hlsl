...
PS_DeferredOutput main(VS_DeferredOutput input)
{
    ...
    float4 radiance = input.Color * lerp(0.25f, 1.0f, 1 - DNBalance);
    
    PS_DeferredOutput output;
    PSFillGBuffer(outColor, input.Depth / FarClip, normal, float4(MaterialProps.x, MaterialProps.y, 0, 1), radiance, output);
