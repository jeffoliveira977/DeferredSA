...
float gameTime()
{
    ...
    time += 0 * 100.0f + 0; //for controlling
	
    time += Time * 6 / 100.0f; //the time flow independent to the time of day
...
inline float ShadowMarching(float dens, float3 p, float2 densityChunk, float3 densityDetail, float3 scaleChunk, float3 scaleDetail, float3 shift, float4 profile, float3 dens_thres, const in float3 offsetA, const in float3 offsetB, const in float3 offsetC, float3 SunDirection, float4 shadow, float2 volumeBox, float2 solidness, float4 distortionParm)
{
    ...
    if (dens <= 0.025)
        return dens * shadow.x; //return a approx. value
...
float4 RenderClouds(float3 ViewDir, float3 ViewPos, float3 sunDir, float distance)
{
    ...
    CloudProfile p = (CloudProfile) 0;
    
    //if (ViewPos.z > 3500.0)
    //    p = cloudProfiles[2];
    //else if (ViewPos.z > 1500.0)
    //    p = cloudProfiles[1];
    //else
        p = cloudProfiles[1];
        
    float4 clouds = CloudAtRay(p, baseColor, ViewDir, ViewPos, light, lightDir, time, distance);
    ...
    cloud_temp = CloudAtRay(p, baseColor, ViewDir, ViewPos, light, lightDir, time, distance);
