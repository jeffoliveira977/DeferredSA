...
sampler2D weatherTex : register(s1);
sampler2D cloudTex : register(s2);
sampler2D worleyTex : register(s3);

row_major float4x4 InvProj : register(c0);
...
static float2 Resolution = {1920.0f, 1080.0f };
static float4 AmbientColor = { 102.0f / 255.0f, 104.0f / 255.0f, 105.0f / 255.0f, 1.0f };
static float4 WindDir = { 1.0f, 0.0f, 0.0f, 1.0f };
static float WindSpeed = 175.0f;;
static float Crispiness = 43.0f;
static float Curliness = 1.1f;
static float Coverage = 0.305f;
static float Absorption = 0.003f;
static float BottomHeight = 2340.0f;
static float TopHeight = 16400.0f;
static float DensityFactor = 0.012f;
...
float SampleCloudDensity(float3 p, bool useHighFreq, float lod)
{
    ...
    float4 lowFreqNoise = tex2Dlod(cloudTex, float4(UV * Crispiness, heightFraction, lod));
    ...
    float3 weatherNoise = tex2D(weatherTex,dynamicUV).rgb;
    ...
    if (useHighFreq)
    {
        float3 highFreqNoise = tex2Dlod(worleyTex, float4(dynamicUV * Crispiness * Curliness, heightFraction * Curliness, lod)).rgb;
